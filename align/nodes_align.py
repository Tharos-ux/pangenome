...
def show_identity(gfa_files: list, gfa_versions: list, colors: list) -> MultiDiGraph:
    ...
    for idx, connex_component in enumerate(graphs):
        all_other_nodes: list = list(chain(graphs[:idx] + graphs[idx+1:]))
        print(all_other_nodes)
        for node in connex_component:
            scores: list = node_aligner(node, all_other_nodes)
            for i, score in enumerate(scores):
                combined_view.add_edge(
                    node, all_other_nodes[i], color='forestgreen', arrows='', alpha=score)
