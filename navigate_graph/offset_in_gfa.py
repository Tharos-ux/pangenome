...
def calculate_sequence_offsets(node_data: list[tuple], walks: list[Record], offsets: list[int] | None = None) -> list[dict]:
    ...
    """
    if offsets is None:
        offsets = [0 for _ in range(len(walks))]
    elif len(offsets) != len(walks):
        raise ValueError(
            "If specified, offset array must be of the same length than walks array.")
